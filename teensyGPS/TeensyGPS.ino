...
/* This demonstrates the normal use of a TinyGPS object.
   It assumes that you have a 9600-baud serial GPS device hooked up
   to a serial port with Rx and Tx pins available.
*/
...
void loop() {
  ...
  if (newData) {
    ...
    if (TinyGPS::GPS_INVALID_F_ANGLE != 0)
      sprintf(lat_lon, "LL=(%.6f,%.6f)", flat, flon);
    Serial.print(" SAT=");
  ...
  sprintf(buf,"Time: %d/%02d/%02d %02d:%02d:%02d", year, month, day, hour, minutes, second);
  Serial.println(buf);
  ...
  tft.setTextSize(2);
  tft.println(buf);
  if (TinyGPS::GPS_INVALID_F_ANGLE != 0.0 ) {
    tft.setCursor(10, 40);
    tft.println(lat_lon);
  }
  delay(1000);
  if (++aspect >= 2) aspect = 0;
}
